...
export default function QuoteFormSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://formsubmit.co/ajax/instinctt20@gmail.com", {
        method: "POST",
        headers: {
    ...
    <section id="contact" className="bg-sky-brand py-12 md:py-32 px-4 sm:px-8 md:px-10">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">
        ...
        <div className="lg:col-span-5 space-y-4 md:space-y-6">
          ...
          <h2 className="font-display font-extrabold text-2xl xs:text-3xl sm:text-5xl md:text-6xl text-maroon-brand leading-tight tracking-tight">
            Book an estimate
          </h2>

          <h3 className="font-display font-bold text-xl sm:text-2xl md:text-3xl text-maroon-brand">
          ...
          <div className="pt-2 md:pt-4 space-y-2.5 text-maroon-brand/85 font-semibold text-sm">
            <div className="flex items-center gap-3">
              <Clock className="w-5 h-5 text-maroon-brand shrink-0" />
              <span>Average response time: &lt; 24 hours</span>
            </div>
            <div className="flex items-center gap-3">
              <CheckCircle className="w-5 h-5 text-maroon-brand shrink-0" />
        ...
        <div className="lg:col-span-7">
          <div className="bg-white rounded-3xl p-5 xs:p-6 sm:p-8 md:p-12 shadow-2xl border border-white/80 relative">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6 md:mb-8">
              <h3 className="font-display font-extrabold text-2xl sm:text-3xl md:text-4xl text-maroon-brand">
                Request your free quote
              </h3>
              <span className="text-[11px] font-bold uppercase tracking-wider bg-pink-brand/60 text-maroon-brand px-3 py-1 rounded-full self-start sm:self-center">
                24h Response
              </span>
            </div>

            {status.submitted ? (
              <div className="bg-green-50/90 border-2 border-green-300 text-green-900 p-6 sm:p-8 rounded-3xl space-y-4 animate-fadeIn">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center shrink-0">
                    <CheckCircle className="w-7 h-7 text-green-600" />
                  </div>
                  <div>
                    <h4 className="font-display font-extrabold text-lg sm:text-xl text-green-900">
                ...
                {status.submittedData && (
                  <div className="bg-white/95 border border-green-200 rounded-2xl p-4 space-y-2 text-xs sm:text-sm shadow-xs">
                ...
                <div className="pt-1">
                  <button
                    ...
                    className="w-full bg-maroon-brand hover:bg-maroon-dark text-white font-bold py-3 rounded-full text-sm transition-all duration-200 cursor-pointer shadow-md text-center"
                  >
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                {/* Name Input */}
                <div>
                  <label htmlFor="name" className="block text-maroon-brand font-bold text-sm sm:text-base mb-1.5 flex items-center gap-2">
                    <User className="w-4 h-4 text-maroon-brand/70" />
                    <span>Your Name</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Email Address Input */}
                  <div>
                    <label htmlFor="email" className="block text-maroon-brand font-bold text-sm sm:text-base mb-1.5 flex items-center gap-2">
                      <Mail className="w-4 h-4 text-maroon-brand/70" />
                  ...
                  <div>
                    <label htmlFor="service" className="block text-maroon-brand font-bold text-sm sm:text-base mb-1.5 flex items-center gap-2">
                      <Layers className="w-4 h-4 text-maroon-brand/70" />
                    ...
                    <select
                      ...
                      onChange={handleChange}
                      className="w-full px-4 sm:px-5 py-3 sm:py-3.5 rounded-xl border border-maroon-brand/20 bg-gray-50/60 text-maroon-brand text-base focus:bg-white focus:border-maroon-brand focus:ring-2 focus:ring-maroon-brand/20 outline-none transition-all duration-200 cursor-pointer"
                    >
                ...
                {/* Message Input */}
                <div>
                  <label htmlFor="message" className="block text-maroon-brand font-bold text-sm sm:text-base mb-1.5 flex items-center gap-2">
                    <MessageSquare className="w-4 h-4 text-maroon-brand/70" />
                  ...
                  <textarea
                    ...
                    className="w-full px-4 sm:px-5 py-3 sm:py-3.5 rounded-xl border border-maroon-brand/20 bg-gray-50/60 text-maroon-brand text-base focus:bg-white focus:border-maroon-brand focus:ring-2 focus:ring-maroon-brand/20 outline-none transition-all duration-200 resize-y"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full min-h-[48px] bg-pink-brand hover:bg-pink-vivid active:scale-[0.99] text-maroon-brand font-display font-extrabold text-lg sm:text-xl py-3.5 sm:py-5 rounded-full shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-75"
                >
                  {status.submitting ? (
                    <div className="flex items-center gap-3">
                ...
                {/* Subtext info */}
                <p className="text-center text-maroon-brand/70 text-xs sm:text-sm mt-3">
